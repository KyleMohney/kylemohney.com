...
-- Uses PostgreSQL sequences for guaranteed unique numbering (like projects!)
...
-- Backfill existing records (calculate what the sequences should start at)
DO $$
DECLARE
  max_client_num INT;
...
BEGIN
  ...
  -- Advance sequences to next number after max
  PERFORM setval('client_serial_seq', max_client_num + 1);
  PERFORM setval('practitioner_serial_seq', max_practitioner_num + 1);
END;
$$;

COMMENT ON COLUMN clients.serial_number IS 'Human-readable client identifier (C1, C2, C3...) - auto-generated by trigger';
COMMENT ON COLUMN practitioners.serial_number IS 'Human-readable practitioner identifier (P1, P2, P3...) - auto-generated by trigger';
...
-- ============================================================================
-- Description: Create notifications table for practitioner alerts
-- Stores: Review posted, message received, match events

CREATE TABLE IF NOT EXISTS notifications (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  practitioner_id UUID NOT NULL REFERENCES practitioners(user_id) ON DELETE CASCADE,
  type TEXT NOT NULL, -- 'review_posted', 'message_received', 'match_event'
  title TEXT NOT NULL,
  message TEXT NOT NULL,
  link TEXT, -- URL to related resource (e.g., /rooted-vitality/dashboard/pro/pages/reviews.html)
  is_read BOOLEAN DEFAULT false,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT now()
);

CREATE INDEX IF NOT EXISTS idx_notifications_practitioner_id ON notifications(practitioner_id);
CREATE INDEX IF NOT EXISTS idx_notifications_created_at ON notifications(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_notifications_is_read ON notifications(is_read);

COMMENT ON TABLE notifications IS 'Notifications for practitioners about platform events';
COMMENT ON COLUMN notifications.practitioner_id IS 'Practitioner user_id receiving the notification';
COMMENT ON COLUMN notifications.type IS 'Type of notification: review_posted, message_received, match_event';
COMMENT ON COLUMN notifications.link IS 'URL to navigate to related resource';
COMMENT ON COLUMN notifications.is_read IS 'Whether practitioner has read this notification';
